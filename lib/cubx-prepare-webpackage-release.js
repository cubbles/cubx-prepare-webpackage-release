...
(function () {
  ...
  WebpackageReleasePreparer.prototype.updateManifestToNextDevVersion = function () {
    if (this.options.hasOwnProperty('nextVersion')) {
      ...
      this.options.nextVersion = this.getDefaultNextDevVersion(this.manifest.version);
    }
   ...
   * @returns {*} - Version of the manifest
   */
  WebpackageReleasePreparer.prototype.getCurrentVersion = function () {
   ...
   * @returns {*} - manifest
   */
  WebpackageReleasePreparer.prototype.getManifest = function () {
    return this.manifest;
  };

  /**
   * Set value of the nextVersion option to be used after releasing the package
   * @param version - Next development version to be set (e.g. 1.2.3-SNAPSHOT)
   */
  WebpackageReleasePreparer.prototype.setNextDevVersion = function (version) {
    if (this.isValidDevVersion(version)) {
      this.options.nextVersion = version;
    } else {
      console.error(
        'WebpackageReleasePreparer: The provided next development version (' + version + ') ' +
        'is not valid, it can only contain numbers or dot separated numbers. And it should ' +
        'finish with the \'-SNAPSHOT\' (e.g. 1.0-SNAPSHOT, 1.3.1-SNAPSHOT, 1-SNAPSHOT)'
      );
      throw new Error('Invalid development version');
  ...
  /**
   * Set value of the releaseVersion option to be used for releasing the package
   * @param version - Release version to be set (e.g. 1.2.3)
   */
  WebpackageReleasePreparer.prototype.setReleaseVersion = function (version) {
    if (this.isValidReleaseVersion(version)) {
      this.options.releaseVersion = version;
    } else {
      console.error(
        'WebpackageReleasePreparer: The provided release version (' + version + ') ' +
        'is not valid, it can only contain numbers or numbers separated by dots ' +
        '(e.g. 1.0, 1.3.1, 1)'
      );
      throw new Error('Invalid release version');
